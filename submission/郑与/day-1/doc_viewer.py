...
class SimpleLark:
        ...
        if isinstance(field_value, str):
            ...
            for item in field_value:
                if isinstance(item, dict):
                    # 如果是字典，尝试提取name或text字段
                    if 'name' in item:
                        text_parts.append(str(item['name']))
                    elif 'text' in item:
                        text_parts.append(str(item['text']))
                    else:
                        # 如果没有name或text，将整个字典转为字符串
    ...
    def find_person_records(self, person_name: str, table_name: str = "default") -> List[Dict[str, Any]]:
        """查找主讲或助教字段中包含指定人员的记录"""
        all_records = self.get_records(table_name)
        matched_records = []
        ...
        for record in all_records:
            ...
            # 检查主讲字段
            instructor_field = fields.get("主讲", "")
            instructor_text = self._extract_field_text(instructor_field)
            
            # 检查助教字段
            assistant_field = fields.get("助教", "")
            assistant_text = self._extract_field_text(assistant_field)
            
            # 如果在主讲或助教中找到指定人员，则添加到结果中
            if (person_name in instructor_text) or (person_name in assistant_text):
                matched_records.append(record)
        
        return matched_records
    
    def print_person_records(self, person_name: str, table_name: str = "default"):
        """查询并打印主讲或助教中包含指定人员的条目"""
        print(f"\n🔍 查询主讲或助教中包含 '{person_name}' 的条目")
        print("=" * 60)
        
        try:
            matched_records = self.find_person_records(person_name, table_name)
            
            if not matched_records:
                print(f"❌ 未找到主讲或助教中包含 '{person_name}' 的记录")
                return
            
            print(f"✅ 找到 {len(matched_records)} 条相关记录：\n")
            ...
            for i, record in enumerate(matched_records, 1):
                ...
                # 显示关键字段
                key_fields = ["课程", "主讲", "助教", "日期", "时间", "地点"]
                
                for field_name in key_fields:
                    if field_name in fields:
                        field_value = fields[field_name]
                        display_text = self._extract_field_text(field_value)
                        
                        # 如果是查询的字段且包含目标人员，则高亮显示
                        if field_name in ["主讲", "助教"] and person_name in display_text:
                            print(f"  🎯 {field_name}: {display_text}")
                        else:
                            print(f"     {field_name}: {display_text}")
...
if __name__ == "__main__":
    # 从环境变量获取配置信息
    app_id = os.environ.get("FEISHU_APP_ID", "cli_a8fd8f9f2ebb101c")
    app_secret = os.environ.get("FEISHU_APP_SECRET", "LbJVHHYqEwi9Mjjp7Ef7YcctHmKLpivP")
    bitable_url = "https://fudan-nlp.feishu.cn/base/KH8obWHvqam2Y4sXGGuct2HFnEb?table=tbljlS1fS0UepxBn&view=vewCig26Kk"
    
    # 检查是否提供了真实凭据
    if app_id == "你的 APP_ID" or app_secret == "你的 APP_SECRET":
